...
NX_STM32_ETH_DIAGNOSTICS g_nx_stm32_eth_diagnostics;
...
static UINT  _nx_driver_hardware_initialize(NX_IP_DRIVER *driver_req_ptr)
{
  ...
  /* BFD=1 means "broadcast frame disable" on STM32F7. DHCP relies on
     broadcast frames, therefore the broadcast filter must remain disabled. */
  ...
  /* Apply a deterministic filter before reception starts. The original ST
     driver only applied this structure after a multicast join/leave. */
...
static UINT  _nx_driver_hardware_enable(NX_IP_DRIVER *driver_req_ptr)
{
  ...
  g_nx_stm32_eth_diagnostics.start_status = (uint32_t)hal_status;
...
void HAL_ETH_TxFreeCallback(uint32_t * buff)
{
  ...
  g_nx_stm32_eth_diagnostics.tx_packets_released++;
...
static VOID  _nx_driver_hardware_packet_received(VOID)
{
  ...
  {
      g_nx_stm32_eth_diagnostics.rx_packets_delivered++;
...
void HAL_ETH_RxAllocateCallback(uint8_t ** buff)
{
  ...
  {
    g_nx_stm32_eth_diagnostics.rx_allocated++;
    /* Adjust the packet.  */
  ...
  {
    g_nx_stm32_eth_diagnostics.rx_alloc_failed++;
...
void HAL_ETH_RxLinkCallback(void **first_packet_ptr, void **last_packet_ptr, uint8_t *buff, uint16_t Length)
{
  ...
  NX_PACKET  *received_packet_ptr;
  const uintptr_t packet_header_size =
      ((sizeof(NX_PACKET) + NX_PACKET_ALIGNMENT - 1U) /
       NX_PACKET_ALIGNMENT) * NX_PACKET_ALIGNMENT;

  /* nx_packet_pool_create() aligns the payload after NX_PACKET. With the
     project's 32-byte packet alignment sizeof(NX_PACKET) is 56 bytes but
     the actual header stride is 64 bytes. Subtracting sizeof(NX_PACKET), as
     the stock ST driver did, points eight bytes inside the real structure
     and corrupts every received packet. */
  uint8_t *data_buffer_ptr = buff - 2U - packet_header_size;

  received_packet_ptr = (NX_PACKET *)data_buffer_ptr;

#if defined (__DCACHE_PRESENT) && (__DCACHE_PRESENT == 1U)
  /* The DMA has just written this buffer. Invalidate it again here so that
     speculative M7 cache fills cannot hide freshly received bytes. */
  SCB_InvalidateDCache_by_Addr((uint32_t *)received_packet_ptr->nx_packet_data_start,
                              received_packet_ptr->nx_packet_data_end -
                              received_packet_ptr->nx_packet_data_start);
#endif

  g_nx_stm32_eth_diagnostics.rx_buffers_linked++;
...
void HAL_ETH_RxCpltCallback(ETH_HandleTypeDef *heth)
{
  ...
  g_nx_stm32_eth_diagnostics.rx_interrupts++;
...
{
  ULONG deffered_events;
  g_nx_stm32_eth_diagnostics.tx_interrupts++;
  deffered_events = nx_driver_information.nx_driver_information_deferred_events;
...
{
  g_nx_stm32_eth_diagnostics.error_interrupts++;
  g_nx_stm32_eth_diagnostics.last_dma_error = heth->DMAErrorCode;
}
