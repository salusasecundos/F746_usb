...
static void SDRAM_Initialization_Sequence(SDRAM_HandleTypeDef *sdram)
{
  ...
  if (HAL_SDRAM_SendCommand(sdram, &command, HAL_MAX_DELAY) != HAL_OK)
  {
    Error_Handler();
  }
  ...
  /* 64 ms / 4096 rows at a 108 MHz SDRAM clock, with a conservative margin. */
  if (HAL_SDRAM_ProgramRefreshRate(sdram, 0x0603U) != HAL_OK)
  {
    Error_Handler();
  }
}
